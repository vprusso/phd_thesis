...
\begin{document}
	...
	\begin{center}
		\begin{tikzpicture}[text depth=1pt,bend angle=45]
			...
			\tikzstyle{Bell}=[shape=rectangle,rounded corners,thick,draw=black!50,thick,fill=white,minimum height=15mm,minimum width=10mm]
			\tikzstyle{CD}=[shape=rectangle,rounded corners,dotted,draw=black!50,thick,fill=gray!35,minimum height=10mm,minimum width=10mm]

			\node[CD] (A) at (-4,1) {\small{Alice}};			
			\node[CD] (B) at (-4,-1) {\small{Bob}};		
			\node[AB] (rho) at (-2,0) {\small{$\phi_{0,0}^{(m)}$}};	
			
			\node[Bell] (Bell) at (1,1) {\large $T$};
			
			\node[Bell] (Pauli) at (5,-1) {\large $X_m^{k_1} Z_m^{k_2}$};
	
			\node (rho) at (7.5,-1) {\small{$\rho$}};
			\node (rho2) at (-1.5,1.5) {\small{$\rho$} };

			%\draw[thick] (-1.5,0.3) [out=0,in=180] to [out=0,in=180] node[above] {$\reg{X}$} (0.5,1);
			%\draw[thick] (-1.5,-0.3) [out=0,in=180] to [out=-30,in=180] node[above] {$\reg{Y}$} (4.3,-1);		
			
			\draw[-,thick] (-1.5,0.5) to node[above] {$\reg{X}$} (0.5,0.5);	
			\draw[-,thick] (-1.5,-0.5) to node[above] {$\reg{Y}$} (4.2,-0.5);
			
			\draw[->,thick] (-1,1.5) to (0.5,1.5);
			
			\draw[-,dashed] (-1.5,0) to (7,0);
			\draw[-,dashed] (-2.5,0) to (-4.5,0);
			
			%\draw[-,thick] (0.5,1.3) to node[above] {$\reg{Z}$} (2.5,1.3);			
			
			%\draw[-,thick] (0.5,0.7) to node[above] {$\reg{X}$} (2.5,0.7);			

			%\draw[-,thick] (0.5,-1) to node[above] {$\reg{Y}$} (Pauli);						
			
			
			
			\draw[double,thick] (Bell) to node[above] {\small{$\quad k_1,k_2 $}} (5.01,1);
			\draw[double,thick] (5,1) to (5,-0.28);
			
			\draw[->,thick] (Pauli) to (7,-1);
