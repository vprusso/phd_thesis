...
\begin{document}
			...
			\begin{tikzpicture}[text depth=1pt, bend angle=45,shorten >=1pt,node distance=2cm,scale=1.0, transform shape]
  			...
  			\tikzstyle {AB}   = [shape=rectangle, rounded corners, dotted, draw=black!50, thick,fill=gray!15, minimum height=6mm, minimum width=6mm] 
				...
				\node[Ref] (Referee) at (2.25,0) {Referee};
				\node[AB] (Alice) at (0,1.5) {\small Alice};
				\node[AB] (Bob) at (4.5,1.5) {\small Bob};

				%\draw[thick,->] (Referee) -- node [right] {\small{$r$}} (2.25,-1);

			 \path[->,thick] (Referee) edge [bend left=45] node [below] {\small{$x$}} (Alice)
                     edge [bend right=45] node [below] {\small{$y$}} (Bob);
                            
                    
			 \path[<-,thick] (Referee) edge [bend right=45] node [above] {\small{$a$}} (Alice)
                     edge [bend left=45] node [above] {\small{$b$}} (Bob);

				\path [-, decoration={zigzag,segment length=4,amplitude=.9,
  post=lineto,post length=2pt},font=\scriptsize,
  line join=round,thick,blue,bend left=45] (Alice) edge[decorate] node [above] {\small{$\rho$}} (Bob);

\path [-, decoration={zigzag,segment length=4,amplitude=.9,
  post=lineto,post length=2pt},font=\scriptsize,
  line join=round,thick,blue]  
				(Alice) edge[decorate] (Referee) 
				(Referee) edge[decorate] (Bob); 
			\end{tikzpicture}
\end{document}
