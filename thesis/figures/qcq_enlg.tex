...
\begin{document}
	...
	\begin{center}
		\begin{tikzpicture}[text depth=1pt,bend angle=45]
			...
			\node[AB] (A-1) at (3,2) {\small $\Phi^x$};
			\node[AB] (B-1) at (3,-2) {\small $\Phi^y$};
...
%			\draw[-,thick] (Ref-0) to node[above]{$\reg{R}$} (Ref-1);
			...
			\draw[-,thick] (3.5,2) [out=90,in=180] to [out=0,in=180] node[above,right] {$\reg{A}$} (5.5,0.5);
			\draw[-,thick] (3.5,-2) [out=90,in=180] to [out=0,in=180] node[above,right] {$\reg{B}$} (5.5,-0.5);										
		\end{tikzpicture}
	\end{center}	
